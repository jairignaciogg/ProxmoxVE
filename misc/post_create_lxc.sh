...
default_setup() {
  ...
  pct exec "$CTID" -- /bin/bash -c "source <(wget -qLO - https://raw.githubusercontent.com/jairignaciogg/ProxmoxVE/fixProxmox9.1/misc/install.func) && color && verb_ip6 && catch_errors && setting_up_container && network_check && update_os" || exit
...
if [[ "${NVIDIA_PASSTHROUGH}" == "yes" ]]; then
  ...
  source <(curl -s https://raw.githubusercontent.com/jairignaciogg/ProxmoxVE/fixProxmox9.1/misc/nvidia.func)
