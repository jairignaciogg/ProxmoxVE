...
default_setup() {
  ...
  pct exec "$CTID" -- /bin/bash -c "source <(wget -qLO - https://raw.githubusercontent.com/jairignaciogg/ProxmoxVE/nvidiapyindexerror/misc/install.func) && color && verb_ip6 && catch_errors && setting_up_container && network_check && update_os" || exit
